...
icon: Wand
description: Generate a typed @adonis-agora/filter-client builder from a FilterSpec — generateFilterClient (pure string transform) and the make:filter-client ace command that writes the modules to disk.
---
...
The generated module targets `@adonis-agora/filter-client`'s `filterQueryTyped` factory — the same runtime the [Client Builder](/docs/filter/guides/client) documents.
