...
<Callout type="warn">
Relation-path **sorting** produces an `orderBy` on the dotted field; ordering across a to-many relation may require the relation to be joined/aggregated for the column to resolve. Relation **filtering** (`whereHas`) needs no join. Prefer relation filters for correctness, and sort by aggregate aliases (e.g. `withCount`) where you need relation-derived ordering — see the [Relations guide](/docs/filter/guides/relations#sorting-by-a-relation-aggregate).
</Callout>
