...
- **Allow-list first.** Operators only matter for fields that survive the allow-list in `applyFilter`. A condition on a disallowed field is dropped before it reaches the adapter. See [Filter Classes](/docs/filter/guides/filter-classes).
