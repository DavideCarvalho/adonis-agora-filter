...
<Callout type="info">
Because the policy is just an object, you can store it next to the model, share it across controllers, or compute it per request (e.g. widen the allow-list for admins). See [Controllers](/docs/filter/guides/controllers) for patterns.
</Callout>
...
import type { FilterConfig } from '@adonis-agora/filter'
